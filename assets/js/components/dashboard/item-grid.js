...
export class ItemGrid extends BaseComponent {
  ...
  /**
   * @typedef {Object} FallbackMessage
   *   @property {Object} [options={}]
   * Optional component configuration.
   *
   * @property {string} [options.icon='bi-inbox']
   * Bootstrap Icons class name.
   *
   * @property {string} [options.title='No content available']
   ...
   * @param {FallbackMessage|null} [options.customFallbackMessage=null] - Fallback message
   */
  constructor({
    data = [],
    onItemClick = null,
    ...
    this.fallbackMessage = customFallbackMessage || {
      icon: "bi-search",
      title: "Nenhum anúncio encontrado",
      description: "Não existem anúncios para exibir.",
    };
  ...
  renderEmptyState() {
    this.emptyState = new EmptyState(this.fallbackMessage);

    const emptyElement = this.emptyState.render();
  ...
  showLoading() {
    ...
    this.loading.show(this.contentElement);
  }
  ...
  clear() {
    ...
    if (this.emptyState?.element && this.emptyState.element.parentNode) {
      this.emptyState.element.parentNode.removeChild(this.emptyState.element);
    }
  ...
  clearContent() {
    ...
    while (this.contentElement && this.contentElement.firstChild) {
